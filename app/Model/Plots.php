...
class Plots extends Model
{
	...
	//   $data = DB::select(DB::raw("SELECT * From plots ORDER By plots.pid ASC"));
                   ...
                   $data = $this->leftJoin('debtors_master', 'plots.debtor_no', '=', 'debtors_master.debtor_no')
                    ->select('plots.*', 'debtors_master.name as cus_name')
    ...
    public function getAllPlotsCsv()
    {
      $dad =  DB::select("SELECT * FROM plots");
      return $dad;
    }
}