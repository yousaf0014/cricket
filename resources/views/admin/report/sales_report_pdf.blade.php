...
<body>
  ...
  <strong>Sales Report(<?php echo formatDate(date('d-m-Y'))?>)</strong> 
  <br> <br>
 ...
 <tr>
   <th>Date</th>
   <th>Sales Volume</th>
   <th>Sales Value({{Session::get('currency_symbol')}})</th>
   <th>No Of Order</th>
 </tr>
      <?php
        $qty = 0;
  ...
  <th>{{ $qty }}</th>
  <th>{{ Session::get('currency_symbol').number_format($price,2,'.',',') }}</td>
  <th>{{ $order }}</th>
